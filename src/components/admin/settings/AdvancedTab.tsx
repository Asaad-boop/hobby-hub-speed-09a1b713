...
import { Card, Loading, Btn, Input, Textarea, Modal, Empty } from "@/components/admin/ui";
...
export function AdvancedTab() {
  ...
  return (
    <Card>
      ...
      >
        <div>
          <div className="text-sm font-semibold">Raw key/value editor</div>
          <div className="text-xs text-gray-500">Advanced — any setting not in the typed UI.</div>
        </div>
              ...
              <tr key={s.key} className="border-t border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-2.5">
                  <div className="flex justify-end gap-1">
                    ...
                    <Btn variant="danger" onClick={() => deleteSetting(s.key)}>
...
function SettingModal({
  ...
  async function save() {
    ...
    const { error } = await supabase
      ...
      .upsert({ key, value: parsed, description } as any, { onConflict: "key" });
    setSaving(false);
    if (error) return toast.error(error.message);
    toast.success("Saved");
    onSaved();
  ...
  return (
    <Modal open onClose={onClose} title="Setting">
      <div className="space-y-3">
        <div>
          <div className="mb-1 text-xs font-medium text-gray-600">Key</div>
          <Input value={key} onChange={(e) => setKey(e.target.value)} disabled={!!s.key} />
        </div>
        <div>
          <div className="mb-1 text-xs font-medium text-gray-600">Value (JSON or string)</div>
